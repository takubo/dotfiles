...
function! BufferList(current)
  ...
  if bufexists('[__BUFFERLIST__]')
    exec 'bwipeout __BUFFERLIST__'
  endif
...
"?        let l:bufname = '...' . strpart(l:bufname, strlen(l:bufname) - g:BufferListMaxWidth + 8)
  ...
  nmap <silent> <buffer> <Esc><Esc> <nop>
  map <silent> <buffer> <Esc><Esc> <nop>
  nunmap <silent> <buffer> <Esc><Esc>
  unmap <silent> <buffer> <Esc><Esc>
...
"?map <buffer> <Right> <Nop>
  ...
  map <silent> <buffer> gg :call BufferListMove(1)<CR>
  map <silent> <buffer> G :call BufferListMove(line("$"))<CR>
